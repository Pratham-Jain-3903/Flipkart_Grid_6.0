...
#                 object_id = f"{CLASSES[idx]}_{int(startX)}_{int(startY)}_{int(endX)}_{int(endY)}"

#                 # Update this line to pass the category (CLASSES[idx])
#                 is_new, count = counter.update(object_id, filtered_state, CLASSES[idx],confidence)

#                 # Save the frame and increment count if it's a new detection or time difference > 0.3 sec
#                 if is_new:
#                     write_to_csv(csv_filename, [datetime.now(), CLASSES[idx], count])

#                     if CLASSES[idx] == "bottle":
#                         # Save the frame if the detected class is 'bottle'
#                         image_path = save_bottle_frame(frame, count, bottle_image_dir)
#                         write_to_csv(csv_filename, [datetime.now(), CLASSES[idx], count, image_path])
...
def main():
    ...
    while True:
        ...
        cv2.putText(frame, f"Count: {counter.count}", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)
