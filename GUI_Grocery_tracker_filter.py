...
class GroceryScannerApp:
    def __init__(self, root):
        ...
        self.mobilenet_detector = MobileNetDetector(
            r"MobileNetSSD_deploy.prototxt.txt",
    ...
    # def process_frame(self, frame):  #intial debugging only
    ...
    # def solve_frame(self, frame): #for testing a time frame based implementation
    ...
    # def process_frame(self, frame): # version 2.0 in progress
    #     try:
    #         # Save the current frame to an image file for processing
    #         timestamp = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
    #         filename = f"frame_{timestamp}.jpg"
